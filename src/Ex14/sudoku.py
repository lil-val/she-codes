...
BOARDS = ['debug', 'n00b', '133t', 'error']  # available sudoku boards
...
def parse_arguments():
    """parses arguments of the form: sudoku.py ,board name"""
    arg_parser = argparse.ArgumentParser()
    arg_parser.add_argument("--board", help="Desired board name", type=str, choices=BOARDS, required=True)
    args = vars(arg_parser.parse_args())  # creates a dictionary of keys = argument flag, and value = argument
    return args['board']
...
class SudokuBoard(object):
    ...
    def __create_board(self, board_file):
        ...
        for line in board_file:
            line = line.strip()
            if len(line) != 9:
                raise SudokuError("Each line must contain 9 characters")
            board.append([])
            for c in line:
                if not c.isdigit():
                    raise SudokuError("Each character must be a number between 0 to 9")
                board[-1].append(int(c))
        if len(board) != 9:
            raise SudokuError("Each puzzle must contain 9 lines")
        return board
...
class SudokuUI(Frame):
    """The Tkinter UI, responsible for drawing the board and accepting user inout"""
    ...
    def __cell_clicked(self, event):
        ...
        if MARGIN < x < (WIDTH - MARGIN) and MARGIN < y < (HEIGHT - MARGIN):
            ...
            row, col = int((y - MARGIN) / SIDE), int((x - MARGIN) / SIDE)  # get row and col from x, y coordinates
            if (row, col) == (self.row, self.col):  # if cell was selected already, deselect it
                self.row, self.col = -1, -1
            else:
